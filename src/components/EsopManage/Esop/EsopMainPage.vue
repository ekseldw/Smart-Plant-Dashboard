...
import AlertCheck from "@/components/AlertWindow/alert-check.vue"
...
export default {
  ...
  beforeCreate() {
    console.log(this.$stompClient.connected,"before");
  },
  created() {
      ...
      this.stompClient.connect(
        ...
        (frame) => {
          ...
          this.stompClient.subscribe("/alert", (res) => {
            ...
            let routeData = this.$router.resolve({path:'/simulation',query:{'situationId':3,'levelId':state}})
            window.open(routeData.href,'_blank');
            //this.$router.push("/simulation?situationId=3&levelId=" + state);
          });
        },
        // errorCallback
        (error) => {
